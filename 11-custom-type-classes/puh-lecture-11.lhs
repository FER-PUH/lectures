...
'Data.Functor' module:
  class Functor f where
    fmap :: (a -> b) -> f a -> f b
    (<$>) :: (a -> b) -> f a -> f b -- infix version of fmap

From the Data.Functor docs on Hackage
...
So, instances of this type class are types that can be mapped over.
All types that serve as data containers (and some that are not) can be mapped over.
Function 'fmap' is a generalization of 'map', which is defined only for lists.
...
Here is an example of non-data-container type that is an instance of Functor.
Using what we learned from Either, what can we do here?
