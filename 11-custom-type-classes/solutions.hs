...
instance Foldable RoseTree where
  ...
  foldr f z (RoseTree t subtrees) = t `f` (foldr (\st acc -> foldr f acc st) z subtrees)
  foldl f z RoseEmpty = z
  foldl f z (RoseTree t subtrees) = foldl (foldl f) z subtrees `f` t

test = RoseTree 1 [RoseTree 2 [], RoseTree 4 []]
