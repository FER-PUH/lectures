...
(c) 2022 Jan Šnajder, Ante Kegalj
(c) 2017 Jan Šnajder
...
However this is in fact how haskell desugars all functions.
...
REMARK 3: Because FP is based on lambda calculus, every program can be written
as a single lambda expression, although no one would really want to do that.
