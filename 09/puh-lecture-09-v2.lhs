...
This, sadly, doesn't allow the compiler to prevents us from misusing the
...
Type synonyms have a purpose, but on their own they don't guarantee type safety.

=== SUM TYPES ==================================================================

Haskell has Algebraic Data Types or ADTs for short. In the algebra of types, our
focus will be on the sum and product types. Let's see a familiar sum type:
...
It should be clearer why they are called the sum types. We can use pattern
matching to react to values of a certain sum type:
...
default, we can only derive Eq, Ord, Enum, Bounded, Read, Show and Ix.
...
when we want to use higher order functions like 'map'. Consider the following:
...
We can now construct a list of 'Coord'inates that represents positions of the
ships in the game of sinking ships:
...
constructor and take the individual coordinates out.
...
We can define some getters to extract individual components a coordinate:
...
relaying on the positions of the fields in the constructor can is annoying. We
...
'Maybe' represents possibly missing value and is an important parametrized type.
...
The problem is that we don't know that without looking at the implementation.
Ideally, we want to communicate such behaviour through types. By doing so, the
compiler can warn us of such edge cases if we forgot to handle them.
...
'Either' represents possibility of two values of different (or same) type.
