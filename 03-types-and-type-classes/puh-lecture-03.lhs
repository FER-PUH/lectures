...
* STRONGLY typed => A vague term programmers use to describe how easy
  it is to bypass the type system's restrictions (e.g., implicit conversions).
...
Functions also have types. E.g., 'words', 'chr', 'ord', 'toUpper'.
...
(1) Type annotations serve as documentation and contracts.
...
So, from now on, you'll be expected to provide type signatures for all
top-level definitions. In most other places, we will rely on Haskell's powerful
type inference:
...
> foo10 w = [x ++ y | x <- words w, y <- words w]
> foo11 w = [(x,y) | x <- words w, y <- words w]
> foo12 w = [y : x | x <- words w, y <- w]
> foo13 w = [(y:x, w) | x <- words w, y <- w]
> foo14 w = [(x, x=='a') | x <- w ]
> foo15 s = tail [ c | c <- s, isLower c ]
> foo16 s = zip [ c | c <- s, isLower c ] "Haskell"
> foo17 n c = reverse $ drop n $ c : "Haskell"
> foo18 xs = last $ lines xs
> foo19 x z = x : 'y' : z
...
possible. This is exactly the type Haskell will infer if you don't specify anything.

In other words, Haskell always infers the most general (polymorphic) type
possible.

Haskell's type inference will detect all type errors before compiling the code.
...
drop: Int -> [a] -> [a], [a] -> Int -> [a], [Bool] -> Int -> [Bool]
reverse: [a] -> [a]
...
Double, Float, Integer, String etc. are not bounded. So this wouldn't work:
